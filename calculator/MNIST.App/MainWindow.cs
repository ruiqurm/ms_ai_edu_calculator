            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (strokePoints.Any())
                {
                    ...
                    foreach (IGrouping<int, StrokeRecord> group in groups)
                    {
                        ...
                        var image = new DenseTensor<float>(new[] { 1, 1, ImageSize, ImageSize });
                        ...
                        var result = new List<NamedOnnxValue> { NamedOnnxValue.CreateFromTensor<float>("input", image) };
                        using (var results = model.Run(result))
                        {
                            var r = results.ToList().Last();
                            var l = r.AsTensor<float>().ToList();
                            var (number, index) = l.Select((n, i) => (n, i)).Max();
                            batchResult.Add(index);
                        }
                    }
                    ...
                    outputText.Text = EvaluateAndFormatExpression(batchResult);
